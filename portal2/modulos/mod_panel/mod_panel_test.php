...
// --------------------------------------------------
// MATRIZ GESTIONES ADICIONALES POR FECHA
// --------------------------------------------------
...
$sqlGestionesMatriz = "
    SELECT
        ...
        COUNT(DISTINCT CONCAT(DATE(r.created_at), '-', COALESCE(r.id_local, v.id_local, 0))) AS total
...
$paramsGM = [$division_filtro, $id_empresa];
$typesGM  = "ii";

if ($subdivision_filtro > 0) {
    $sqlGestionesMatriz .= " AND f.id_subdivision = ?";
    $typesGM .= "i";
    $paramsGM[] = $subdivision_filtro;
}
...
if (!empty($fecha_desde) && !empty($fecha_hasta)) {
    ...
    $sqlGestionesMatriz .= " AND r.created_at BETWEEN ? AND ?";
    $typesGM .= "ss";
    $paramsGM[] = $inicio;
    $paramsGM[] = $fin;
}
...
$sqlGestionesMatriz .= "
    GROUP BY r.id_usuario, fecha
";

$stmtGM = $conn->prepare($sqlGestionesMatriz);
$stmtGM->bind_param($typesGM, ...$paramsGM);
$stmtGM->execute();
$resGM = $stmtGM->get_result();

while ($row = $resGM->fetch_assoc()) {
    ...
    $matriz[$id]['datos'][$fecha] =
        ($matriz[$id]['datos'][$fecha] ?? 0) + $valor;
}
...
$(document).ready(function() {
    ...
    // Cuando cambia divisi車n
    $('[name="division"]').on('change', function() {
        let divisionId = $(this).val();
        cargarSubdivisiones(divisionId);
    });

    // Si viene subdivision seleccionada al cargar p芍gina
