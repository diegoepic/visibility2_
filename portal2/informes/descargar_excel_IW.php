...
// ------------------------------------------------------------------------------------
// 1) Entradas y validaciones b��sicas
// ------------------------------------------------------------------------------------
$formulario_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
$inline        = isset($_GET['inline']) && $_GET['inline'] === '1';

if ($formulario_id <= 0) {
    die("ID de formulario inv��lido.");
}
...
// Empresa en sesi��n (multi-cuenta)
...
$questionIds = [];   // ordered list of question IDs (unique)
...
//    - Fallback puntual a "usuario|minuto" s��lo si no hay visita (hist��rico raro)
//    - Para iw_requiere_local=1 se a�0�9adir�� Direcci��n/Nombre/C��digo Local
// ------------------------------------------------------------------------------------
...
while ($row = $rs->fetch_assoc()) {
    ...
    // Guardar respuesta por ID de pregunta (evita mezclar preguntas con igual texto)
    $data[$eventKey]['answers'][$qid][] = [
...
foreach ($data as $event) {
    ...
    // 1) calcular cu��ntas filas necesita este evento
    $maxRows = 1;
    ...
    for ($i = 0; $i < $maxRows; $i++) {
        ...
        foreach ($questionIds as $qid) {
            ...
            // si existe respuesta para esta pregunta e ��ndice
            if (isset($answers[$qid][$i])) {
                $row[$colKey] = $answers[$qid][$i]['answer'] ?? '';

                if ($isValued[$qid]) {
                    $row[$colKey . '_val'] = $answers[$qid][$i]['valor'] ?? '';
                }
            }
        }
...
if (!empty($data)) {
    foreach ($data as $r) {
        foreach ($r as $col => $val) {
            if (strpos($col, 'q_') === 0 && substr($col, -4) === '_val') {
                if (trim((string)$val) !== '') {
                    $valorCols[$col] = true;
                } else {
...
$dynamic    = [];   // display names for headers
$dynamicKeys = [];  // internal keys for data lookup
...
foreach ($questionIds as $qid) {
    ...
    if ($isValued[$qid]) {
        // solo incluir columna Valor si no fue eliminada por estar toda vacía
        if (!isset($valorCols[$valKey]) || $valorCols[$valKey]) {
            $dynamic[]    = $txt . ' (Valor)';
            $dynamicKeys[] = $valKey;
        }
    }
...
$headerKeys = array_merge($fixedHeaders, $dynamicKeys);
