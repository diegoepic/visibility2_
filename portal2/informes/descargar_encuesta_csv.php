...
function fail(string $message): void
{
    ...
    error_log('[EXPORT ENCUESTA] ' . $message);
...
function obtenerFormularioBase(mysqli $conn, int $idForm): array
{
    ...
    if (!$stmt->bind_param('i', $idForm)) {
        $err = $stmt->error;
        $stmt->close();
        fail('Error bind formulario: ' . $err);
    }

    if (!$stmt->execute()) {
        $err = $stmt->error;
        $stmt->close();
        fail('Error ejecutando formulario: ' . $err);
    }
    ...
    if ($res === false) {
        $err = $stmt->error;
        $stmt->close();
        fail('Error obteniendo resultado formulario: ' . $err);
    }
    ...
    if (!$row) {
        fail('Formulario no encontrado. ID=' . $idForm);
    }
...
function getEncuestaPivot(mysqli $conn, int $idForm): array
{
    ...
    if (!$stmtQ->bind_param('i', $idForm)) {
        $err = $stmtQ->error;
        $stmtQ->close();
        fail('Error bind preguntas encuesta: ' . $err);
    }
    ...
    if ($resQ === false) {
        $err = $stmtQ->error;
        $stmtQ->close();
        fail('Error obteniendo resultado preguntas encuesta: ' . $err);
    }
...
";
    ...
    if (!$stmt->bind_param('i', $idForm)) {
        $err = $stmt->error;
        $stmt->close();
        fail('Error bind getEncuestaPivot: ' . $err);
    }
    ...
    while ($row = $res->fetch_assoc()) {
        $fechaKey = $row['fecha_visita'] ?? 'SIN_FECHA';
