...
// Requiere login (el guard auto_prepend ya valida, esto es por si cae .user.ini)
...
while ($row = $result->fetch_assoc()) {
    $row['campanasIds'] = $row['campanasIds'] !== null && $row['campanasIds'] !== ''
        ? explode(',', $row['campanasIds'])
        : [];
    $locales[] = [
...
<head>
    ...
    #success-alert {
        position: fixed; top: 60px; left: 0; width: 100%; z-index: 9999;
    ...
    .circulo {
        ...
        width: 30px; height: 30px; background-color: #28a745; color: #fff;
        border-radius: 50%; font-weight: bold; font-size: 14px;
    }
    #panelInstruccionesModal {
        position: absolute; top: 60px; right: 10px; width: 300px; max-height: 80%;
        overflow-y: auto; background-color: rgba(255, 255, 255, 0.95);
    ...
    @media (max-width: 768px) {
        ...
        #panelInstruccionesModal {
            right: 50%; transform: translateX(50%); width: 90%; left: 5%; top: auto; bottom: 60px;
        }
    }
    .custom-map-control-button {
        background-color: #fff; border: 2px solid #fff; border-radius: 3px;
        box-shadow: 0 2px 6px rgba(0,0,0,0.3); cursor: pointer; margin: 10px;
        padding: 10px; font-size: 16px; font-family: 'Roboto, Arial, sans-serif';
    ...
    #loadingIndicator {
        ...
        background-color: rgba(255, 255, 255, 0.8); padding: 5px 10px; border-radius: 3px;
                                    ...
                                    <tbody>
                                        ...
                                        <tr data-idlocal="<?php echo $idLocal; ?>"
                                            ...
                                            class="<?php echo $trClass; ?>">
                                             <td class="center"><?php echo htmlspecialchars($row['codigoLocal'], ENT_QUOTES, 'UTF-8'); ?></td>
                                             <td class="center">
                                                <?php if ($esPrioridad) { ?><i class="fa fa-star priority-icon" title="Local prioritario"></i><?php } ?>
                                                <?php echo htmlspecialchars($cadena, ENT_QUOTES, 'UTF-8'); ?>
                                             </td>
                                             <td><?php echo htmlspecialchars($direccionLocal, ENT_QUOTES, 'UTF-8'); ?></td>
                                             <td class="center">
...
<?php
// Mantengo la lógica original de generar modals por local (no se rompe nada)
$localesTotales = array_merge($locales, $locales_reag);
$idsGenerados = [];
foreach ($localesTotales as $row) {
    if (in_array($row['idLocal'], $idsGenerados, true)) { continue; }
        ...
        <div class='modal-content'>
          <div class='modal-header'>
            ...
            <h4 class='modal-title' id='myModalLabel{$idLocal}'>
              Local: ".htmlspecialchars($codigoLocal,ENT_QUOTES,'UTF-8')." - ".htmlspecialchars($nombreLocal,ENT_QUOTES,'UTF-8')."<br>
              Dirección: ".htmlspecialchars($direccionLocal,ENT_QUOTES,'UTF-8')."<br>
              Vendedor: ".htmlspecialchars($vendedor,ENT_QUOTES,'UTF-8')."
            </h4>
          </div>
...
<!-- Scripts (mantengo CDNs para no romper nada; luego los pasamos a locales para offline total) -->
...
window.initMap = function() {
    let usarRutas = false; // mantener desactivado por ahora
    let coordenadasProg = <?php echo json_encode($coordenadas_locales_programados, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); ?>;
    let coordenadasReag = <?php echo json_encode($coordenadas_locales_reag, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); ?>;
    ...
    coordenadasProg.forEach(function(local) {
        ...
        let marker = new google.maps.Marker({
            position: position, map: window.mapa, title: local.nombre_local,
        ...
        marker.addListener('click', function() { infoWindow.open(window.mapa, marker); });
        window.markersProg[local.idLocal] = { marker: marker, fechaPropuesta: local.fechaPropuesta };
    });
    ...
    coordenadasReag.forEach(function(local) {
        ...
        let infoWindow = new google.maps.InfoWindow({ content: contentString });
        marker.addListener('click', function() { infoWindow.open(window.mapa, marker); });
        window.markersReag[local.idLocal] = { marker: marker, fechaPropuesta: local.fechaPropuesta };
    });

    // Por defecto: mostrar programados, ocultar reagendados
    window.modoLocal = 'prog';
    for (let id in window.markersReag) {
        if (Object.prototype.hasOwnProperty.call(window.markersReag, id)) {
            window.markersReag[id].marker.setMap(null);
        }
    }

    window.ejecutorMarker = new google.maps.Marker({
        position: { lat: -33.4489, lng: -70.6693 }, map: window.mapa,
    ...
    window.calcularRutaOptima = function(origen) {
        ...
        window.directionsService.route(request, function(result, status) {
            if (status === google.maps.DirectionsStatus.OK) {
                ...
                if (window.ultimaRuta === nuevaRuta) { return; }
    ...
    if (navigator.geolocation) {
        navigator.geolocation.watchPosition(
            function (position) {
                ...
                if (debeCentrarMapa(ubicacionActual)) { window.mapa.panTo(ubicacionActual); }
    ...
    setTimeout(function(){ $('#filtroFechaProg').trigger('change'); }, 500);
};
...
function applyFilters() {
  ...
  const tachadas = $('.todo .completed').map((i, li) => String($(li).data('idcampana'))).get();
  ...
  $(`${container} table[data-fechaTabla]`).each(function() {
    ...
    $(this).find('tbody tr').each(function() {
      const camps = String($(this).data('campanas')||'').split(',').filter(Boolean);
      const ok    = camps.length === 0 ? true : camps.some(c => !tachadas.includes(c));
      $(this).data('ok', ok);
      if (ok) tieneOk = true;
    });
...
$(document).ready(function(){
  ...
  $(document).on('click', '.todo-actions', function(){
    const $li = $(this).closest('li'), $i  = $li.find('i');
...
function updateCounts() {
  let panelSelector = window.modoLocal === 'prog' ? '#localesProgCollapse' : '#localesReagCollapse';
  let countTabla = $(panelSelector + ' table[data-fechaTabla] tbody tr:visible').length;
  $('#countTabla').text(countTabla);

  let markers = window.modoLocal === 'prog' ? window.markersProg : window.markersReag;
  let countMapa = Object.values(markers).filter(m => (m.marker.getMap() !== null)).length;
  $('#countMapa').text(countMapa);
}
...
function filtrarLocalesTabla(modo) {
  ...
  $tablas.each(function(){
    ...
    $tbodyRows.each(function(){
      ...
      const campanas = String($(this).data('campanas') || '').split(',').filter(Boolean);
      const okCamp   = campanas.length === 0 ? true : campanas.some(c => !tachadas.includes(c));

      const matchTxt = (codigo.includes(filtro) || cadena.includes(filtro) || comuna.includes(filtro) || direccion.includes(filtro));
      const visible  = okCamp && matchTxt;
      $(this).toggle(visible);
      if (visible) { visiblesEnEstaTabla++; }
    });
...
<!-- Bootstrap offline inline: guarda un snapshot de datos de inicio por usuario -->
<script id="v2-bootstrap" type="application/json">
<?php
echo json_encode([
  'user_id' => $usuario_id,
  'campanas' => $campanas,
  'compCampanas' => $compCampanas,
  'locales_por_dia' => $locales_por_dia,
  'locales_reag_por_dia' => $locales_reag_por_dia
...
(function(){
  try {
    var el = document.getElementById('v2-bootstrap');
    if (!el) return;
    var data = JSON.parse(el.textContent || el.innerText || '{}');
    if (!data || !data.user_id) return;
    var key = 'v2_bootstrap_' + data.user_id;
    localStorage.setItem(key, JSON.stringify({ts: Date.now(), data: data}));
  } catch(e) { /* noop */ }
})();
</script>

<!-- Registrar Service Worker (no invasivo, si ya está registrado no hace nada) -->
<script>
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('/visibility2/sw.js', { scope: '/visibility2/' }).catch(function(){});
}
</script>
<!-- Google Maps API -->
<script async defer
