...
| validaci車n de sesi車n web del portal.
*/
if (
...
if (!function_exists('assert_session_is_valid')) {
   function assert_session_is_valid(mysqli $conn): void {
    ...
    if ($st) {
      ...
      if ($st->execute()) {
        ...
        if ($row && !is_null($row['revoked_at'])) {
          // Sesi車n revocada 〞 no hay bypass, ni para cola offline
          session_unset();
          session_destroy();
          if ($wantsJson) {
            http_response_code(401);
    ...
    // Inactividad: si last_seen_at supera el umbral, cerrar sesi車n
      ...
      if ($inactSt) {
        ...
        if ($inactRow && (int)$inactRow['idle_min'] > $inactivityMinutes) {
          ...
          if ($wantsJson) {
            http_response_code(401);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['ok' => false, 'error_code' => 'SESSION_EXPIRED', 'message' => 'Sesi車n expirada por inactividad', 'retryable' => false], JSON_UNESCAPED_UNICODE);
            exit;
          }
          header('Location: /visibility2/app/login.php?session_expired=1'); exit;
        }
