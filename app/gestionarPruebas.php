...
function setupFileInput(inputElem) {
    ...
    for (const prep of prepared) {
      ...
      try {
        const res = await QueueObj.smartPost('/visibility2/app/upload_material_foto_pruebas.php', fd, {
          ...
          if (res.queued) {
            appendMaterialHidden(idFQ, `queued://${effId}`, effId);
  ...
  function appendMaterialHidden(idFQ, url, jobId) {
    const hiddenContainer = document.getElementById(`hiddenUploadContainer_${idFQ}`);
    if (!hiddenContainer) return;

    let hiddenInput = null;
    if (jobId) {
      hiddenInput = hiddenContainer.querySelector(`input[type="hidden"][data-job-id="${jobId}"]`);
    }
    ...
    if (!hiddenInput) {
      ...
      if (jobId) hiddenInput.dataset.jobId = jobId;
  ...
  function handleMaterialUploadResponse(jobId, resp) {
    ...
    appendMaterialHidden(info.idFQ, normalizeAppUrl(finalUrl), jobId);
