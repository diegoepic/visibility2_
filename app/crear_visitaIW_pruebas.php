...
// === Permisos + requisitos del formulario (incluye flag requiere local) ===
...
$stmt->execute();
$stmt->bind_result($formEmpresaId, $formDivisionId, $requiereLocal);
$ok = $stmt->fetch();
$stmt->close();
...
// Validar id_local si la campaña IW lo requiere
$id_local = 0;
if ((int)$requiereLocal === 1) {
  ...
  // Validar que el local pertenece a la empresa (y división si aplica)
  if ((int)$formDivisionId > 0) {
    $q = $conn->prepare("SELECT id FROM local WHERE id=? AND id_empresa=? AND id_division=? LIMIT 1");
    $q->bind_param("iii", $id_local, $empresaId, $formDivisionId);
  } else {
  ...
  if (!$hasLocal) {
    http_response_code(403);
    echo json_encode(['status'=>'error','message'=>'Local inválido para esta campaña']); exit;
  }
}
...
// === Crear visita (id_local puede ser 0 o seleccionado) ===
